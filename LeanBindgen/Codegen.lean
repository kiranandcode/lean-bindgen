...
partial def resolveType
  ...
  | .typedef name =>
    ...
    | some a =>
      ...
      | .variadicBuilder _ => .ok (opaqueRT a.lean name)
    | none =>
...
def emitLeanModule (b : Bindings) (hdr : CHeader) : Except String String := do
  ...
  for ta in b.types do
    ...
    | .mutableStruct msm =>
      ...
      let _ := cTypeName
    | _ => pure ()
  -- Variadic builder decls
  let mut builderDecls : Array Gen.LeanDecl := #[]
  for ta in b.types do
    match ta.mapping with
    | .variadicBuilder vbm =>
      for ctor in vbm.constructors do
        let mut argParts : Array String := #[]
        for arg in ctor.args do
          let leanTy := match arg.kind with
            | .number   => arg.enumType.getD "Int32"
            | .symbol   => if vbm.symbolLean.isEmpty then "UInt64" else vbm.symbolLean
            | .location => vbm.locationLean
            | .string   => "String"
            | .ast      => vbm.resultType
            | .optionalAst => s!"Option {vbm.resultType}"
            | .stringArray => "Array String"
            | .astArray    => s!"Array {vbm.resultType}"
          argParts := argParts.push s!"@& {leanTy}"
        let errTy := match vbm.errorReturn with
          | .string _ => "String"
          | .enum _ eLean => eLean
          | .tuple _ eLean _ => s!"({eLean} × String)"
        let retTy := s!"IO (Except {errTy} {vbm.resultType})"
        let sigParts := " → ".intercalate argParts.toList
        let fullSig := if argParts.isEmpty then retTy else s!"{sigParts} → {retTy}"
        let externSym := s!"lean_{toSnake ctor.leanName}"
        builderDecls := builderDecls.push (.externOpaque externSym ctor.leanName fullSig)
    | _ => pure ()
  -- Function decls
  ...
  let allDecls := typeDecls ++ mutableDecls ++ builderDecls ++ fnDecls ++ constDecls
...
/-- Emit C shim helpers for a variadic builder: one fixed-arity C function
per constructor that calls the variadic C function with the correct
arguments. -/
private def emitVariadicBuilderHelpers
    (_h : HeaderIndex) (_anno : Std.HashMap String TypeAnno)
    (_ta : TypeAnno) (vbm : VariadicBuilderMapping)
    : Except String (Array Gen.CTopLevel) := do
  let resultClassGetter := externalClassGetter vbm.resultType
  let locToC := if vbm.locationLean.isEmpty then ""
                else (structHelperNames vbm.locationLean).1
  let mut tops : Array Gen.CTopLevel := #[]
  for ctor in vbm.constructors do
    let externSym := s!"lean_{toSnake ctor.leanName}"
    -- Build C parameter list
    let mut cParams : Array String := #[]
    for arg in ctor.args do
      let paramTy := match arg.kind with
        | .number   =>
          match arg.enumType with
          | some _ => "uint8_t"
          | none   => "int32_t"
        | .symbol   => "uint64_t"
        | .location => "b_lean_obj_arg"
        | .string   => "b_lean_obj_arg"
        | .ast      => "b_lean_obj_arg"
        | .optionalAst => "b_lean_obj_arg"
        | .stringArray => "b_lean_obj_arg"
        | .astArray    => "b_lean_obj_arg"
      cParams := cParams.push s!"{paramTy} {arg.leanName}"
    -- Build extraction prelude + variadic arg list
    let mut prelude : Array String := #[]
    let mut varArgs : Array String := #[]
    let mut freeStmts : Array String := #[]
    for arg in ctor.args do
      match arg.kind with
      | .number =>
        match arg.enumType with
        | some eLean =>
          let (toLeanToC, _) := enumHelpers eLean
          prelude := prelude.push s!"  int {arg.leanName}_v = (int){toLeanToC}({arg.leanName});"
          varArgs := varArgs.push s!"{arg.leanName}_v"
        | none =>
          varArgs := varArgs.push s!"(int){arg.leanName}"
      | .symbol =>
        varArgs := varArgs.push s!"(clingo_symbol_t){arg.leanName}"
      | .location =>
        prelude := prelude.push s!"  {vbm.locationCType} {arg.leanName}_c = {locToC}({arg.leanName});"
        varArgs := varArgs.push s!"&{arg.leanName}_c"
      | .string =>
        prelude := prelude.push s!"  const char *{arg.leanName}_c = lean_string_cstr({arg.leanName});"
        varArgs := varArgs.push s!"{arg.leanName}_c"
      | .ast =>
        prelude := prelude.push s!"  {vbm.resultCType}_t *{arg.leanName}_c = ({vbm.resultCType}_t *)lean_get_external_data({arg.leanName});"
        varArgs := varArgs.push s!"{arg.leanName}_c"
      | .optionalAst =>
        prelude := prelude.push
          (s!"  {vbm.resultCType}_t *{arg.leanName}_c = " ++
           s!"lean_obj_tag({arg.leanName}) == 0 ? NULL : " ++
           s!"({vbm.resultCType}_t *)lean_get_external_data(lean_ctor_get({arg.leanName}, 0));")
        varArgs := varArgs.push s!"{arg.leanName}_c"
      | .stringArray =>
        prelude := prelude ++ #[
          s!"  size_t {arg.leanName}_size = lean_array_size({arg.leanName});",
          s!"  char const **{arg.leanName}_buf = (char const **)malloc({arg.leanName}_size * sizeof(char const *));",
          s!"  for (size_t _i = 0; _i < {arg.leanName}_size; _i++) \{",
          s!"    {arg.leanName}_buf[_i] = lean_string_cstr(lean_array_get_core({arg.leanName}, _i));",
          s!"  }"
        ]
        varArgs := varArgs ++ #[s!"{arg.leanName}_buf", s!"{arg.leanName}_size"]
        freeStmts := freeStmts.push s!"  free({arg.leanName}_buf);"
      | .astArray =>
        prelude := prelude ++ #[
          s!"  size_t {arg.leanName}_size = lean_array_size({arg.leanName});",
          s!"  {vbm.resultCType}_t **{arg.leanName}_buf = ({vbm.resultCType}_t **)malloc({arg.leanName}_size * sizeof({vbm.resultCType}_t *));",
          s!"  for (size_t _i = 0; _i < {arg.leanName}_size; _i++) \{",
          s!"    {arg.leanName}_buf[_i] = ({vbm.resultCType}_t *)lean_get_external_data(lean_array_get_core({arg.leanName}, _i));",
          s!"  }"
        ]
        varArgs := varArgs ++ #[s!"{arg.leanName}_buf", s!"{arg.leanName}_size"]
        freeStmts := freeStmts.push s!"  free({arg.leanName}_buf);"
    ...
    let preludeStr := "\n".intercalate prelude.toList
    let freeStr := "\n".intercalate freeStmts.toList
    let allVarArgs := #[ctor.enumValue, "&_ast"] ++ varArgs
    let callStr := s!"{vbm.variadicFn}({", ".intercalate allVarArgs.toList})"
    -- OK block: wrap _ast in Lean external, Except.ok
    let okBlock :=
      (if freeStr.isEmpty then "" else freeStr ++ "\n") ++
      s!"    lean_object *_ext = lean_alloc_external({resultClassGetter}(), _ast);\n" ++
      s!"    lean_object *ok = lean_alloc_ctor(1, 1, 0);\n" ++
      s!"    lean_ctor_set(ok, 0, _ext);\n" ++
      s!"    return lean_io_result_mk_ok(ok);"
    -- Error block: reuse ErrorReturn pattern
    let errBlock := match vbm.errorReturn with
      | .string errMsgFn =>
        (if freeStr.isEmpty then "" else freeStr ++ "\n") ++
        s!"    char const *msg = {errMsgFn}();\n" ++
        s!"    if (msg == NULL) msg = \"\";\n" ++
        s!"    lean_object *err = lean_alloc_ctor(0, 1, 0);\n" ++
        s!"    lean_ctor_set(err, 0, lean_mk_string(msg));\n" ++
        s!"    return lean_io_result_mk_ok(err);"
      | .enum codeFn eLean =>
        let (_, toLeanFn) := enumHelpers eLean
        (if freeStr.isEmpty then "" else freeStr ++ "\n") ++
        s!"    lean_object *code = lean_box({toLeanFn}({codeFn}()));\n" ++
        s!"    lean_object *err = lean_alloc_ctor(0, 1, 0);\n" ++
        s!"    lean_ctor_set(err, 0, code);\n" ++
        s!"    return lean_io_result_mk_ok(err);"
      | .tuple codeFn eLean msgFn =>
        let (_, toLeanFn) := enumHelpers eLean
        (if freeStr.isEmpty then "" else freeStr ++ "\n") ++
        s!"    lean_object *code = lean_box({toLeanFn}({codeFn}()));\n" ++
        s!"    char const *msg = {msgFn}();\n" ++
        s!"    if (msg == NULL) msg = \"\";\n" ++
        s!"    lean_object *pair = lean_alloc_ctor(0, 2, 0);\n" ++
        s!"    lean_ctor_set(pair, 0, code);\n" ++
        s!"    lean_ctor_set(pair, 1, lean_mk_string(msg));\n" ++
        s!"    lean_object *err = lean_alloc_ctor(0, 1, 0);\n" ++
        s!"    lean_ctor_set(err, 0, pair);\n" ++
        s!"    return lean_io_result_mk_ok(err);"
    let paramStr := ", ".intercalate cParams.toList
    let fullParamStr := if paramStr.isEmpty then "lean_obj_arg _unit"
                        else s!"{paramStr}, lean_obj_arg _unit"
    let fn :=
      s!"LEAN_EXPORT lean_obj_res {externSym}({fullParamStr}) \{\n" ++
      (if preludeStr.isEmpty then "" else preludeStr ++ "\n") ++
      s!"  {vbm.resultCType}_t *_ast;\n" ++
      s!"  if ({callStr}) \{\n" ++
      okBlock ++ "\n" ++
      s!"  } else \{\n" ++
      errBlock ++ "\n" ++
...
def emitShim (b : Bindings) (hdr : CHeader) : Except String String := do
  ...
  for ta in b.types do
    ...
    | .variadicBuilder vbm =>
      opaqueHelpers := opaqueHelpers ++ (← emitVariadicBuilderHelpers h typeAnnoMap ta vbm)
    | _ => pure ()
  ...
  let hasVariadicBuilder := b.types.any (fun ta => match ta.mapping with | .variadicBuilder _ => true | _ => false)
  let needsStdlib := hasStructOrTU || hasNestedCallbacks || hasByteArrayOut || hasMutableStruct || hasVariadicBuilder
