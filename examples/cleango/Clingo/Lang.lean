...
def getEndPos? (s: Lean.SourceInfo) : Option String.Pos.Raw :=
  match s with
...
def mkLocation (s : Syntax) : TermElabM Expr := do
        ...
        let locStx :=
               `(Clingo.Generated.ClingoBindings.Location.mk
...
-- Ast.Term.Data.Symbol now takes Symbol.Repr directly, no conversion needed.
...
| `(clingo_term|  ( $l:clingo_term,* ) ) => do
   ...
   mkTermPool (l.getElems[0]!) args.toList
...
| `(clingo_head_aggregate_element| $t:clingo_term,* ) => do
   ...
   let lit <- mkLiteralDefaultTrue (t.getElems[0]!)
